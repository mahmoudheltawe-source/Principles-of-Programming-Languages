...
;;                (<sexp>+ . <sexp>) | ( <sexp>* )       ##### L3
*/
export const check_double_key = (param: Sexp[]): boolean => {
    const keys = map((p: Sexp) => {
        if (isNonEmptyList(p) && isString(p[0])) return p[0];
        throw new Error(`Invalid key in dict: ${JSON.stringify(p)}`);
    }, param);
    return new Set(keys).size === keys.length;
};
export type Exp = DefineExp | CExp;
export type AtomicExp = NumExp | BoolExp | StrExp | PrimOp | VarRef;
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | DictExp; //*****new  */
export type CExp = AtomicExp | CompoundExp;
////***new */
export type DictExp = { tag: "DictExp"; map: DictMap[]; }
export type DictMap = { tag: "DictMap"; key: SymbolSExp; val: CExp; }
...
export const isDictMap = (x: any): x is DictMap => x.tag === "DictMap";
...
const parseDictExp = (params: Sexp[]): Result<DictExp> =>
    mapv(mapResult(parseDictMap, params), (entries: DictMap[]) => makeDictExp(entries));

const parseDictMap = (sexp: Sexp): Result<DictMap> =>
    isNonEmptyList<Sexp>(sexp) && sexp.length === 2
        ? (isString(first(sexp)) && isIdentifier(first(sexp))  
            ? bind(parseL32CExp(second(sexp)), (val: CExp) =>
                makeOk({ tag: "DictMap", key: makeSymbolSExp(first(sexp) as string), val }))
...
export const makeDictMap = (key: SymbolSExp, val: CExp): DictMap => ({ tag: "DictMap", key, val });

export const makeDictExp = (entries: DictMap[]): DictExp =>
    ({ tag: "DictExp", map: entries });
const isValidDictEntries = (params: Sexp[]): boolean =>
    isArray(params) &&
    allT<NonEmptyList<Sexp>>(isNonEmptyList, params) &&
    allT(isIdentifier, map(first, params));
                                ...
                                isAppExp(exp) ? `(${unparseL32(exp.rator)} ${unparseLExps(exp.rands)})` :
                                    isPrimOp(exp) ? exp.op :
                                        isLetExp(exp) ? unparseLetExp(exp) :
                                            isDictExp(exp) ? `(dict ${map((d: DictMap) => `(${d.key.val} ${unparseL32(d.val)})`, exp.map).join(" ")})` :
