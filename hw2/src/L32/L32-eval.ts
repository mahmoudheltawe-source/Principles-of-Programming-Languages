// L32-eval.ts
import { find, map, none } from "ramda";
import { DictMap, DictExp, isCExp, isDictExp, isLetExp } from "./L32-ast";
import {
...
import { isClosure, makeClosure, Closure, Value, DictEntryValue, makeDictValue, isDictValue, DictValue, isSymbolSExp, isCompoundSExp } from "./L32-value";
...
const L32applyProcedure = (proc: Value, args: Value[], env: Env): Result<Value> =>
    isPrimOp(proc) ? applyPrimitive(proc, args) :
        isClosure(proc) ? applyClosure(proc, args, env) :
            isDictValue(proc) ? findDictValue(proc, args) :
                makeFailure(`Bad procedure ${format(proc)}`);

const findDictValue = (dict: DictValue, args: Value[]): Result<Value> =>
    args.length === 1
...
export const evalDict = (exp: DictExp, env: Env): Result<Value> =>
    bind(mapResult((entry: DictMap) => evalmap(entry, env), exp.map),(entries) => makeOk(makeDictValue(entries)));

export const evalmap =(entry: DictMap,env: Env): Result<DictEntryValue> =>
    bind(L32applicativeEval(entry.val, env), (value: Value) =>
        makeOk({key: entry.key,value: value}));
