...
export const parseL31Program = (sexp: Sexp): Result<Program> =>
    ...
    isNonEmptyList<Sexp>(sexp) ? parseL31GoodProgram(first(sexp), rest(sexp)) :
...
const parseL31GoodProgram = (keyword: Sexp, body: Sexp[]): Result<Program> =>
    keyword === "L31" && !isEmpty(body) ? mapv(mapResult(parseL31Exp, body), (exps: Exp[]) => 
                                              makeProgram(exps)) :
    makeFailure(`Program must be of the form (L31 <exp>+): ${format([keyword, ...body])}`);

// Exp -> <DefineExp> | <Cexp>
export const parseL31Exp = (sexp: Sexp): Result<Exp> =>
    isCompoundSexp(sexp) ? 
        isNonEmptyList<Sexp>(sexp) ? parseL31CompoundExp(first(sexp), rest(sexp)) :
...
export const parseL31CExp = (sexp: Sexp): Result<CExp> =>
    isCompoundSexp(sexp) ? 
        isNonEmptyList<Sexp>(sexp) ? parseL31CompoundCExp(first(sexp), rest(sexp)) :
...
import { isSymbolSExp, isEmptySExp, isCompoundSExp } from './L31-value';
