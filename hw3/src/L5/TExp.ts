...
import { Empty, isEmpty, isNonEmptyList } from "../shared/list";
...
//the  new type for pair
// ========================================================
export type PairTExp = { tag: "PairTExp"; left: TExp; right: TExp };
export const makePairTExp = (left: TExp, right: TExp): PairTExp => ({ tag: "PairTExp", left: left, right: right });
...
export type CompoundTExp = ProcTExp | TupleTExp | PairTExp;//add the pair 
...
//export type PairEXP = nonEmptyPair | EmptyPair;//add the pait tybe
...
// EmptyPair: Empty Pair
export type EmptyPair = { tag: "EmptyPair" }
export const makeEmptyPair = (): EmptyPair => ({ tag: "EmptyPair" });
export const isEmptyPair = (x: any): x is EmptyPair => x.tag === "EmptyPair";

// NonEmptyTupleTExp(TEs: NonTupleTExp[])
export type NonEmptyTupleTExp = { tag: "NonEmptyTupleTExp"; TEs: NonTupleTExp[]; }
//export type nonEmptyPair = { tag: "nonEmptyPair"; pair: [NonTupleTExp, NonTupleTExp]; }//non emp pair <non tuble , non tuble >
export const makeNonEmptyTupleTExp = (tes: NonTupleTExp[]): NonEmptyTupleTExp =>
    ({ tag: "NonEmptyTupleTExp", TEs: tes });
// export const makeNonEmptyPair = (pair: [NonTupleTExp, NonTupleTExp]): nonEmptyPair =>
//     ({ tag: "nonEmptyPair", pair: pair });
export const isNonEmptyTupleTExp = (x: any): x is NonEmptyTupleTExp => x.tag === "NonEmptyTupleTExp";
//export const isNonEmptyPair = (x: any): x is nonEmptyPair => x.tag === "nonEmptyPair";//check if non empty pair 
            ...
            (texp === "void") ? makeOk(makeVoidTExp()) :
                (texp === "string") ? makeOk(makeStrTExp()) :
                    isString(texp) ? makeOk(makeTVar(texp)) :
                        isArray(texp) ?
                            parseCompoundTExp(texp) : makeFailure(`Unexpected TExp - ${format(texp)}`);
...
const parseCompoundTExp = (texps: Sexp[]): Result<CompoundTExp> => {
    ...
    return (texps[0] == "Pair" && (pos === -1)) ? parsePairTExp(texps)
...
/*
;; Expected structure:  pair<t1> <t2>
;; Or: Empty
*/
export const parsePairTExp = (texps: Sexp[]): Result<PairTExp> =>
    (texps.length !== 3) ?
        makeFailure(`Expected Pair2 elements, got ${format(texps)}`) :
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    // const unparsePair = (pair: PairTExp): Result<string> =>
    //     bind(unparseTExp(pair.left), (car: string) =>
    //         bind(unparseTExp(pair.right), (cdr: string) =>
    //             makeOk(`(Pair ${car} ${cdr})`)))
    // // isNonEmptyList<TExp>(pair) ?
    // //     bind(unparseTExp(pair[0]), (f: string) =>
    // //         bind(unparseTExp(pair[1]), (s: string) => 
    // //             makeOk([f, '.', s]))) :
    // //     makeOk(["Empty"]);
                                    ...
                                    isEmptyTupleTExp(x) ? makeOk("Empty") :
                                        isNonEmptyTupleTExp(x) ? unparseTuple(x.TEs) :
                                                ...
                                                x === undefined ? makeFailure("Undefined TVar") :
                                                        ...
                                                        isPairTExp(x) ? bind(unparseTExp(x.left), (car: string) =>
                        bind(unparseTExp(x.right), (cdr: string) =>
                                    makeOk(`(Pair ${car} ${cdr})`))) :
                                                    x;
