...
import { is, join, map, zipWith } from "ramda";
...
const isQuoted = (exp: Sexp): boolean =>
  isCompoundSExp(exp) && exp[0] === 'quote';

const isShorthandQuote = (exp: Sexp): boolean =>
  isSymbolSExp(exp) && exp.val.startsWith("'");
// sexps has the shape (quote <sexp>)
export const parseLitExp = (param: Sexp): Result<LitExp> =>
//     isQuoted(param) ?
//     bind(parseSExp(second(param)), (sexp: SExpValue) => makeLitExp(sexp)) :
// :    isShorthandQuote(param) ?
//     parseLitExp(["quote", parse(sexp.val.substring(1))]):
